...
function renderAuth() {
  ...
  if (state.isLoggedIn) {
    ...
    if (state.userRole === "admin") {
      ...
      userManagementDiv.className =
        "bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 mt-6";
    ...
    if (state.userRole === "admin") {
      ...
      profileManagementDiv.className =
        "bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700";
                    ...
                    <div>
                        <select id="admin-profile-player-select" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                            ...
                            ${state.players
                              ...
                              .map(
                                (p) =>
                                  `<option value="${p.id}">${p.name} (${p.team ? `Team ${p.team}` : "No Team"})</option>`,
                              )
                              .join("")}
                        </select>
                    </div>
      ...
      setTimeout(() => {
        document
          .getElementById("admin-tab-users")
          ?.addEventListener("click", () => {
            document
              .getElementById("admin-panel-users")
              .classList.remove("hidden");
            document
              .getElementById("admin-panel-players")
              .classList.add("hidden");
            document
              .getElementById("admin-panel-seasons")
              .classList.add("hidden");
          });
        document
          .getElementById("admin-tab-players")
          ?.addEventListener("click", () => {
            document
              .getElementById("admin-panel-players")
              .classList.remove("hidden");
        ...
        document
          .getElementById("admin-tab-seasons")
          ?.addEventListener("click", () => {
            document
              .getElementById("admin-panel-seasons")
              .classList.remove("hidden");
        ...
        document
          .getElementById("player-tab-roster")
          ?.addEventListener("click", () => {
            document
              .getElementById("player-panel-roster")
              .classList.remove("hidden");
            document
              .getElementById("player-panel-profiles")
              .classList.add("hidden");
            document
              .getElementById("player-panel-archived")
              .classList.add("hidden");
          });
        document
          .getElementById("player-tab-profiles")
          ?.addEventListener("click", () => {
            document
              .getElementById("player-panel-profiles")
              .classList.remove("hidden");
        ...
        document
          .getElementById("player-tab-archived")
          ?.addEventListener("click", () => {
            document
              .getElementById("player-panel-archived")
              .classList.remove("hidden");
        ...
        document
          .getElementById("admin-profile-player-select")
          ?.addEventListener("change", (e) => {
            ...
            if (playerId) {
              ...
              if (player) {
                document.getElementById("admin-profile-edit-name").value =
                  player.name || "";
                document.getElementById("admin-profile-edit-nickname").value =
                  player.nickname || "";
                document.getElementById("admin-profile-edit-team").value =
                  player.team || "";
        ...
        document
          .getElementById("admin-profile-edit-cancel")
          ?.addEventListener("click", () => {
            document.getElementById("admin-profile-player-select").value = "";
            document
              .getElementById("admin-profile-edit-section")
              .classList.add("hidden");
          });

        document
          .getElementById("admin-profile-edit-save")
          ?.addEventListener("click", async () => {
            const playerId = document.getElementById(
              "admin-profile-player-select",
            ).value;
            const newName = document
              .getElementById("admin-profile-edit-name")
              .value.trim();
            const newNickname = document
              .getElementById("admin-profile-edit-nickname")
              .value.trim();
            const newTeam =
              document.getElementById("admin-profile-edit-team").value || null;

            if (!playerId || !newName) {
              showToast(
                "Please select a player and provide a name.",
                "warning",
              );
              return;
            }
        ...
        document
          .getElementById("admin-team-filter")
    ...
    logoutBtn.className =
      "w-full bg-red-500 text-white py-2 px-4 rounded-xl hover:bg-red-600 font-medium";
    ...
    returnBtn.className =
      "w-full bg-gray-800 text-white py-2 px-4 rounded-xl font-medium";
  ...
  for (const tabKey in ui.tabs) {
    ...
    if (tabKey === "setup") {
      ...
      if (state.userRole === "admin") {
        ...
        hasActiveMatch =
          state.fixture.id !== null && state.fixture.team === state.userTeam;
      }
...
async function handleUpdateProfile() {
  ...
  if (state.userRole !== "admin")
    return showToast("Only admins can update player profiles.", "error");
  if (!playerId)
    return showToast("Please select a profile to update.", "error");
...
async function handleConfirmation() {
  ...
  if (action === "cancelMatch") {
    ...
    action === "deleteUpcomingFixture" ||
    action === "deletePreviousFixture"
  ) {
    try {
      await safeFirebaseCall("deleteFixture", async () => {
        await deleteDoc(doc(db, FIXTURES_COLLECTION, data));
      });
      if (state.selectedPreviousFixtureId === data)
        state.selectedPreviousFixtureId = null;
      if (state.selectedUpcomingFixtureId === data)
...
function setupFirestoreListeners() {
  ...
  const seasonsUnsub = onSnapshot(
    query(collection(db, SEASONS_COLLECTION)),
    (snapshot) => {
  ...
  const fixturesUnsub = onSnapshot(
    query(collection(db, FIXTURES_COLLECTION)),
    (snapshot) => {
      ui.connectionStatus.classList.add("hidden");
      ...
      const activeFixture = allFixtures.find(
        (f) => f.status === "live" && f.team === userTeam,
      );
      ...
      if (activeFixture) {
        ...
        state.lastTurnSeq =
          state.fixture.games[state.currentGameIndex]?.turnSeq || 0;
      } else {
        state.fixture = {
          id: null,
          games: [],
      ...
      state.previousFixtures = allFixtures
        ...
        .sort((a, b) => {
          const dateA = a.scheduledDate?.toMillis
            ? a.scheduledDate.toMillis()
            : a.createdAt?.toMillis() || 0;
          const dateB = b.scheduledDate?.toMillis
...
function init() {
  ...
  window.addEventListener("pageshow", (event) => {
    if (event.persisted) handleAppReturn();
  });
  ...
  document
    .querySelector('[data-role="viewer"]')
    .addEventListener("click", () => {
      state.userRole = "viewer";
      state.isLoggedIn = false;
  ...
  document
    .getElementById("email-signin-btn")
  ...
  document
    .getElementById("signup-modal-cancel")
    .addEventListener("click", closeSignupModal);
  document
    .getElementById("signup-modal-submit")
  ...
  document.addEventListener("click", (e) => {
    if (e.target.id === "cancel-match-btn") {
      openConfirmModal(
        "Cancel Match?",
        "Are you sure you want to cancel this match? All current progress will be lost.",
        "cancelMatch",
        null,
      );
    }
  ...
  ui.prevGameBtn.addEventListener("click", () => {
    if (state.currentGameIndex > 0) {
      ...
      render();
      // Note: updateGameData requires db which is internal to scoring.js in previous setups, but works fine here
      // updateGameData();
    }
  });
  ...
  ui.liveMatchContent.addEventListener("click", (e) => {
    ...
    if (statBtn) {
      triggerHaptic("light");
      const playerIndex = statBtn.hasAttribute("data-player-index")
        ? parseInt(statBtn.dataset.playerIndex)
        : 0;
      updateStat(
        statBtn.dataset.stat,
        parseInt(statBtn.dataset.op),
        playerIndex,
      );
    }
  });
  ...
  ui.finesList.addEventListener("click", (e) => {
    ...
    if (payBtn) {
      openConfirmModal(
        "Clear Fines?",
        "Are you sure you want to mark outstanding fines as paid?",
        "clearPlayerFines",
  ...
  document
    .getElementById("team-filter-select")
    ?.addEventListener("change", (e) => {
  ...
  document
    .getElementById("fixtures-view-team-a")
    .addEventListener("click", () => {
      state.activeMatchView = "A";
      updateTeamSwitcherStyles("fixtures");
      renderFixturesTab();
  ...
  document
    .getElementById("fixtures-view-team-b")
    .addEventListener("click", () => {
      state.activeMatchView = "B";
      updateTeamSwitcherStyles("fixtures");
      renderFixturesTab();
    });
  document.getElementById("setup-view-team-a").addEventListener("click", () => {
    state.activeMatchView = "A";
    updateTeamSwitcherStyles("setup");
    renderFixtureSetup();
  });
  document.getElementById("setup-view-team-b").addEventListener("click", () => {
    state.activeMatchView = "B";
    updateTeamSwitcherStyles("setup");
    renderFixtureSetup();
  });
