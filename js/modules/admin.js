...
export function renderAdminPlayerRoster() {
  ...
  const teamFilter =
    document.getElementById("admin-team-filter")?.value || "all";
  ...
  if (activePlayers.length === 0) {
    list.innerHTML =
      '<p class="text-gray-500 dark:text-gray-400 text-sm">No players in roster.</p>';
    return;
  }
  ...
  activePlayers.forEach((player) => {
    ...
    playerEl.className =
      "flex items-center justify-between bg-gray-100 dark:bg-gray-700 p-2 rounded-lg";
    playerEl.innerHTML = `
  ...
  if (tempPlayers.length > 0) {
    ...
    warningDiv.className =
      "bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3 mb-3";
...
export function renderArchivedPlayers() {
  ...
  if (archivedPlayers.length === 0) {
    list.innerHTML =
      '<p class="text-gray-500 dark:text-gray-400 text-sm">No archived players.</p>';
    return;
  }
  ...
  archivedPlayers.forEach((player) => {
    ...
    const playerEl = document.createElement("div");
    playerEl.className =
      "flex items-center justify-between bg-gray-100 dark:bg-gray-700 p-2 rounded-lg opacity-75";
    playerEl.innerHTML = `
            <div class="flex-1 min-w-0">
                <p class="font-medium dark:text-white truncate">${player.name}</p>
                <p class="text-xs text-gray-500 dark:text-gray-400">${player.nickname || "No nickname"} • ${teamLabel}</p>
  ...
  onSnapshot(
    ...
    (snapshot) => {
      ...
      snapshot.docs.forEach((docSnap) => {
        ...
        userItem.className =
          "flex flex-col sm:flex-row sm:items-center justify-between bg-gray-100 dark:bg-gray-600 p-3 rounded-lg space-y-2 sm:space-y-0";
    ...
    if (targetUserData.playerId) {
      await safeFirebaseCall("updatePlayerTeam", async () => {
        return await updateDoc(
          doc(db, PLAYERS_COLLECTION, targetUserData.playerId),
          {
            team: newTeam,
          },
        );
...
export async function updateUserPlayer(userId, newPlayerId) {
  try {
    ...
    const oldPlayer = state.players.find(
      (p) => p.id === targetUserData.playerId,
    );
    const newPlayer = newPlayerId
      ? state.players.find((p) => p.id === newPlayerId)
      : null;
    ...
    if (newPlayerId && targetUserData.team) {
      await safeFirebaseCall("updatePlayerTeam", async () => {
        return await updateDoc(doc(db, PLAYERS_COLLECTION, newPlayerId), {
