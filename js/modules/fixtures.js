...
export function renderFixturePlayerSelectors() {
  ...
  if (state.userRole === "admin") {
    const teamAFixture = state.allFixtures.find(
      (f) => f.status === "live" && f.team === "A",
    );
    const teamBFixture = state.allFixtures.find(
      (f) => f.status === "live" && f.team === "B",
    );
  ...
  GAME_TITLES.forEach((title, i) => {
    ...
    const currentP1Name = currentP1Id
      ? state.players.find((p) => p.id === currentP1Id)?.name || ""
      : "";
    const currentP2Name = currentP2Id
      ? state.players.find((p) => p.id === currentP2Id)?.name || ""
      : "";
...
export function setupPlayerSelectors() {
  ...
  inputs.forEach((input) => {
    ...
    const dropdown = document.getElementById(
      `game-${gameIndex}-${playerSlot}-dropdown`,
    );
...
export function showPlayerDropdown(input, dropdown, players, searchTerm) {
  const filtered = players.filter(
    (p) => p.name.toLowerCase().includes(searchTerm) && !p.archived,
  );
  ...
  if (
    searchTerm &&
    !filtered.find((p) => p.name.toLowerCase() === searchTerm)
  ) {
    const customOption = document.createElement("div");
    customOption.className =
      "px-3 py-2 hover:bg-emerald-100 dark:hover:bg-emerald-900 cursor-pointer border-b border-gray-200 dark:border-gray-700";
...
export function renderFixtureSetup() {
  const isDisabled =
    ...
    state.userRole === "scorer" ||
    (!state.userTeam && state.isLoggedIn && state.userRole !== "viewer");
  ...
  if (state.userRole === "admin" && !hasActiveFixture) {
    const existingNoMatchWarning = document.getElementById(
      "admin-no-match-warning",
    );
    ...
    warningDiv.className =
      "col-span-2 p-4 rounded-xl bg-blue-100 dark:bg-blue-900/30 text-center mb-4";
  ...
  if (!state.userTeam && state.isLoggedIn && state.userRole !== "viewer") {
    ...
    const warningDiv = document.createElement("div");
    warningDiv.className =
      "col-span-2 p-4 rounded-xl bg-yellow-100 dark:bg-yellow-900/30 text-center mb-4";
    warningDiv.innerHTML = `
        ...
        `;
    if (
      ui.fixtureForm.parentElement &&
      !document.getElementById("no-team-warning")
    ) {
  ...
  document
    .querySelectorAll(
      "#tab-content-setup input:not(#opposition-name-input):not(#match-date-input), #tab-content-setup select",
    )
    .forEach((el) => {
  ...
  if (hasActiveFixture) {
    ...
    if (!updateBtn) {
      ...
      updateBtn.className =
        "w-full bg-blue-600 text-white py-3 px-4 rounded-xl hover:bg-blue-700 transition-colors duration-200 font-semibold text-lg";
      updateBtn.textContent = "Update Team Selections";
      ui.createFixtureBtn.parentNode.insertBefore(
        updateBtn,
        ui.createFixtureBtn.nextSibling,
      );
    ...
    if (state.userRole === "admin") {
      ...
      if (isDisabled) {
        ...
        ui.createFixtureBtn.classList.remove(
          "opacity-50",
          "cursor-not-allowed",
        );
      }
...
export function renderCurrentMatch() {
  ...
  const teamFixtures = (state.allFixtures || []).filter(
    (f) => f.team === state.activeMatchView,
  );
  ...
  if (!activeFixture && !state.fixture.id) {
    ...
    if (nextMatch) {
      const matchDate = nextMatch.scheduledDate?.toDate
        ? nextMatch.scheduledDate.toDate()
        : new Date(nextMatch.scheduledDate);
      ui.currentMatchResults.innerHTML = `
  ...
  if (
    !displayFixture &&
    state.fixture.id &&
    state.fixture.team === state.activeMatchView
  ) {
...
export function renderFixturesTab() {
  if (state.activeFixturesTab === "upcoming") {
    ui.fixturesTabUpcoming.classList.add(
      "text-emerald-600",
      "border-emerald-500",
    );
    ui.fixturesTabUpcoming.classList.remove(
      "text-gray-500",
      "border-transparent",
    );
    ui.fixturesTabPrevious.classList.add("text-gray-500", "border-transparent");
    ui.fixturesTabPrevious.classList.remove(
      "text-emerald-600",
      "border-emerald-500",
    );
    ...
    ui.fixturesTabPrevious.classList.add(
      "text-emerald-600",
      "border-emerald-500",
    );
    ui.fixturesTabPrevious.classList.remove(
...
export function renderUpcomingFixtures() {
  ...
  ui.addFixtureBtn.classList.toggle(
    "hidden",
    state.userRole === "viewer" || state.userRole === "scorer",
  );

  const teamFixtures = state.upcomingFixtures.filter(
    (f) => f.team === state.activeMatchView,
  );

  if (teamFixtures.length === 0) {
    ui.upcomingFixturesList.innerHTML =
      '<p class="text-gray-500 dark:text-gray-400">No upcoming matches.</p>';
    return;
  }
  ...
  teamFixtures.forEach((fixture) => {
    ...
    const canStartMatch =
      state.userRole === "admin" || state.userRole === "member";
    const scheduledDate = fixture.scheduledDate?.toDate
      ? fixture.scheduledDate.toDate()
      : new Date(fixture.scheduledDate);
    ...
    item.innerHTML = `
            <div class="flex justify-between items-center">
                ...
                <div class="flex items-center space-x-2 flex-shrink-0">
                    ...
                    ${
                      isAdmin
                        ? `<button data-fixture-id="${fixture.id}" class="delete-upcoming-fixture-btn text-red-500 hover:text-red-700 p-2 rounded-full hover:bg-red-100 dark:hover:bg-red-900/50">
...
export function renderPreviousFixtures() {
  ...
  const teamFixtures = state.previousFixtures.filter(
    (f) => f.team === state.activeMatchView,
  );
  if (teamFixtures.length === 0) {
    ui.previousFixturesList.innerHTML =
      '<p class="text-gray-500 dark:text-gray-400">No finished matches.</p>';
    return;
  }
    ...
    item.innerHTML = `
            <div class="flex justify-between items-center">
                ...
                <div class="flex items-center space-x-2 flex-shrink-0">
                    ...
                    ${
                      isAdmin
                        ? `<button data-fixture-id="${fixture.id}" class="delete-previous-fixture-btn text-red-500 hover:text-red-700 p-2 rounded-full hover:bg-red-100 dark:hover:bg-red-900/50">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                    </button>`
                        : ""
                    }
  ...
  if (type === "upcoming" && canEdit) {
    ...
    if (type === "previous") {
      ...
      fixture.games.forEach((game) => {
        const playerNames = game.playerIds
          .map(
            (id) => state.players.find((p) => p.id === id)?.name || "Unknown",
          )
          .join(" & ");
        modalContent += `
...
export async function saveFixtureEdit(fixtureId) {
  const opposition = validateInput(
    document.getElementById("edit-fixture-opposition").value,
  );
...
export async function addScheduledFixture() {
  ...
  if (!state.activeSeasonId) {
    showToast(
      "Please set an active season before adding a fixture.",
      "warning",
    );
    return;
  }
...
export function showAddFixtureModal() {
  ...
  modal.className =
    "fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50 p-4";
  const isAdmin = state.userRole === "admin";
  ...
  document.getElementById("fixture-date").value = new Date()
    .toISOString()
    .split("T")[0];

  document
    .getElementById("add-fixture-close")
    .addEventListener("click", closeAddFixtureModal);
  document
    .getElementById("add-fixture-cancel")
    .addEventListener("click", closeAddFixtureModal);
  document
    .getElementById("add-fixture-submit")
    .addEventListener("click", submitAddFixture);

  document
    .getElementById("fixture-location")
    .addEventListener("change", (e) => {
      const addressContainer = document.getElementById(
        "fixture-address-container",
      );
...
export async function submitAddFixture() {
  const oppositionName = validateInput(
    document.getElementById("fixture-opposition").value,
  );
  ...
  try {
    ...
    await safeFirebaseCall("addScheduledFixture", async () => {
      return await addDoc(collection(db, FIXTURES_COLLECTION), {
...
export async function startMatchFromScheduled(fixtureId) {
  ...
  const scheduledFixture = state.upcomingFixtures.find(
    (f) => f.id === fixtureId,
  );
  if (!scheduledFixture) {
...
export async function getOrCreatePlayer(nameInput) {
  ...
  const existingByName = state.players.find(
  ...
  try {
    const docRef = await safeFirebaseCall("createPlayer", async () => {
      return await addDoc(collection(db, PLAYERS_COLLECTION), {
        name: trimmedName,
        nickname: "",
        team: state.userTeam,
        stats: {},
...
export async function createFixture() {
  ...
  if (!state.activeSeasonId) {
    showToast(
      "Please set an active season before creating a match.",
      "warning",
    );
    return;
  }
  ...
  try {
    const fixtureTeam =
      state.userRole === "admin" ? state.activeMatchView : state.userTeam;
...
export async function cancelMatch() {
  ...
  try {
    const fixtureRef = doc(db, FIXTURES_COLLECTION, fixtureToCancel.id);
...
export async function finishMatch(dotdPlayerId) {
  ...
  try {
    state.fixture.games.forEach((game) => {
      if (!game.playerNames) {
        game.playerNames = game.playerIds.map(
          (id) => state.players.find((p) => p.id === id)?.name || "Unknown",
        );
      }
    });
    ...
    await runTransaction(db, async (transaction) => {
      const allPlayerIds = [
        ...new Set(state.fixture.games.flatMap((game) => game.playerIds)),
      ];
      ...
      const matchAuditData = {
        ...
        seasonId: seasonId,
        seasonName:
          state.seasons.find((s) => s.id === seasonId)?.name ||
          "Unknown Season",
        ...
        dotdWinner: dotdPlayerId
          ? {
              ...
              playerName:
                state.players.find((p) => p.id === dotdPlayerId)?.name ||
                "Unknown",
            }
          : null,
        gameResults: state.fixture.games.map((game) => ({
          title: game.title,
          type: game.type,
          playerNames: game.playerIds.map(
            (id) => state.players.find((p) => p.id === id)?.name || "Unknown",
          ),
      ...
      for (let i = 0; i < allPlayerIds.length; i++) {
        ...
        state.fixture.games.forEach((game) => {
          ...
          seasonStats.gamesWon =
            (seasonStats.gamesWon || 0) + (gameWon ? 1 : 0);
          seasonStats.gamesLost =
            (seasonStats.gamesLost || 0) + (!gameWon ? 1 : 0);
          seasonStats.legsWon =
            (seasonStats.legsWon || 0) + (game.legsWon || 0);
          seasonStats.legsLost =
            (seasonStats.legsLost || 0) + (game.legsLost || 0);
          seasonStats.scores100 =
            (seasonStats.scores100 || 0) + (pScores.scores100 || 0);
          seasonStats.scores140 =
            (seasonStats.scores140 || 0) + (pScores.scores140 || 0);
          seasonStats.scores180 =
            (seasonStats.scores180 || 0) + (pScores.scores180 || 0);

          if (gameType === "singles") {
            const singlesSeasonStats = newStats[seasonId].singles;
            singlesSeasonStats.outstandingFines =
              (singlesSeasonStats.outstandingFines || 0) + (game.fines || 0);
            singlesSeasonStats.totalFines =
              (singlesSeasonStats.totalFines || 0) + (game.fines || 0);
            ...
            singlesSeasonStats.outstandingFines =
              (singlesSeasonStats.outstandingFines || 0) + playerFines;
            singlesSeasonStats.totalFines =
              (singlesSeasonStats.totalFines || 0) + playerFines;
          }
        });
        ...
        if (dotdPlayerId === playerId) {
          const singlesSeasonStats = newStats[seasonId].singles;
          singlesSeasonStats.outstandingFines =
            (singlesSeasonStats.outstandingFines || 0) + 250;
          singlesSeasonStats.totalFines =
            (singlesSeasonStats.totalFines || 0) + 250;
        }
